...
x1				DCD	  0					; posicion x de la cabeza del jugador 1                       ; se puede poner en byte (DCB)
...
inicio  
				...
				mov r2,#0	
				mov r3,#0
limpia			LDR r0,=inicio_pan		; r0=@inicio_pan=0x40007E00
				mov r1,#' '				; r1='X'
				...
				; poscion inicial jugador 1 random
				mov r1,#31				; valor maximo random posicion X
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				POP {r2}				; desapilar numero random posicion X
				mov r1,#15				; valor maximo random poscion Y
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				POP {r3}				; desapilar numero random posicionY
				
				; dibujar jugador 1 pantalla inicial 
				mov r1,#'X'				; r1='X'
				ldr r4,=x1				; r4=@x1
				ldr r5,=y1				; r5=@y1
				ldr r6,=dir1x			; r6=@dir1x
				ldr r7,=dir1y			; r7=@dir1y
				PUSH {r1-r7}			; apilar ascii,x1,y1,@x1,@y1,@dir1x,@dir1y
				bl dibujar				; llamar a dibujar
				add sp,sp,#28			; desapilar parametros
				
				; poscion inicial jugador 2 random
				mov r1,#31				; valor maximo random posicion X
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				...
				; dibujar jugador 2 pantalla inicial 
				mov r1,#'O'				; r1='X'
				ldr r4,=x2				; r4=@x2
				ldr r5,=y2				; r5=@y2
				ldr r6,=dir2x			; r6=@dir1x
				ldr r7,=dir2y			; r7=@dir1y
				PUSH {r1-r7}			; apilar ascii,x2,y2,@x2,@y2,@dir2x,@dir2y
				bl dibujar				; llamar a dibujar
				add sp,sp,#28			; desapilar parametros
				
				; direccion random jugador 1
				mov r1,#2				; valor maximo random de direccion
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				POP {r1}				; desapilar numero random de direccion
					
				sub r1,r1,#1			; numRandom=numRandom-1
				ldr r2,=dir1x			; r2=@dir1x
				strb r1,[r2]			; dir1x=numRandom
				cmp r1,#0				; if numRandom != 0
				bne direccion2			; saltar a la direccion 2
				
				mov r1,#1				; valor maximo random de direccion
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				POP {r1}				; desapilar numero random de direccion
				...
				sub r1,r1,#1			; numRandom=numRandom-1
				cmp r1,#0				; if numRandom=0
				add r1,r1,#1			; numRandom=numRandom+1
				ldr r2,=dir1y			; r2=@dir1y
				str r1,[r2]				; dir1y=numRandom
				

direccion2		; direccion random jugador 2
				mov r1,#2				; valor maximo random de direccion
				PUSH {r1}				; apilar valor maximo
				bl num_random			; llamar a num_random
				add sp,sp,#4			; libera parametros
				...
				sub r1,r1,#1			; numRandom=numRandom-1
				ldr r2,=dir2x			; r2=@dir1x
				strb r1,[r2]			; dir1x=numRandom
				cmp r1,#0				; if numRandom != 0
				bne direccion2x			; saltar a la direccion 2
				...
				cmp r1,#0				; if numRandom=0
				add r1,r1,#1			; numRandom=numRandom+1
				ldr r2,=dir2y			; r2=@dir1y
				str r1,[r2]				; dir1y=numRandom
				
		
direccion2x		LDR r0,=fin
bucle  			ldr r1,[r0]				; r1=terminar
				...
				; coger direccion del jugador 1
				ldr r1,=dir1x			; r1=@dir1x
				ldrsb r4,[r1]			; r4=dir1x
				ldr r3,=x1				; r3=@x1
				ldr r2,[r3]				; r2=x1
				add r2,r2,r4			; r2=r2+r4 ;x1=x1+dir1x
				and r2,#31				; empieza por el otro lado (x1 en modulo 31)
				
				ldr r1,=dir1y			; r1=@dir1y
				ldrsb r4,[r1]			; r4=dir1y
				ldr r5,=y1				; r5=@y1
				ldr r3,[r5]				; r3=y1
				add r3,r3,r4			; r3=r3+r4 ;y1=y1+dir1y
				and r3,#15				; empieza por el otro lado (y1 en modulo 31)
				...
				; coger direccion del jugador 2
				ldr r1,=dir2x			; r1=@dir2x
				ldrsb r4,[r1]			; r4=dir2x
				ldr r3,=x2				; r3=@x2
				ldr r2,[r3]				; r2=x2
				add r2,r2,r4			; r2=r2+r4 ;x2=x2+dir2x
				and r2,#31				; empieza por el otro lado (x2 en modulo 31)
				
				ldr r1,=dir2y			; r1=@dir2y
				ldrsb r4,[r1]			; r4=dir2y
				ldr r5,=y2				; r5=@y2
				ldr r3,[r5]				; r3=y2
				add r3,r3,r4			; r3=r3+r4 ;y2=y2+dir2y
				and r3,#15				; empieza por el otro lado (y2 en modulo 31)
...
num_random 		PUSH 	{lr,r11}		; apilar @ret y r11
				mov 	fp,sp			; r11=fp=frame pointer
				PUSH 	{r0-r3}			; apilar registros
				
				ldr 	r1,[fp,#8]		; r1=valor maximo
				add 	r0,r0,r1		; r0=r0+r1
				add 	r2,r2,r3		; r2=r2+r3
				add 	r0,r0,r2		; r0=r0+r2
				PUSH 	{r0}			; apilar r0=semilla
				bl 		srand			; llama a srand
				; no libero parametros por que va a ser el espacio para los resultados
				bl 		rand			; llama a rand
				POP 	{r0}			; desapilar numero aliatorio
				and 	r0,r1			; numero aleatorio en modulo valor maximo
				str 	r0,[fp,#12]		; almacenar numero aleatorio
				
				POP 	{r0-r3,r11,pc}	; desapilar registros, r11 y pc
				
dibujar			PUSH 	{r11,lr}		; apilar @ret y r11
				mov 	fp,sp			; r11=fp=frame pointer
				PUSH    {r0-r8}			; apilar registros
				
				ldr 	r2,[fp,#12]		; r2=valor del X
				ldr 	r3,[fp,#16]		; r3=valor del Y
				ldr 	r4,[fp,#20]		; r4=@x
				ldr 	r5,[fp,#24]		; r5=@y
				...
				mul 	r7,r3,r6		; r7=r3*r6=y*32
				add 	r8,r7,r2		; r8=r3*r6+r2=y*32+x	
				add 	r8,r0,r8		; r8=@inicio_pan+r8
				ldrb 	r7,[r8]			; r7=posicion	
				cmp 	r7,#' '			; if r7 != ' '
				bne 	buscar_libre	; saltar a buscar libre
fin_dibujar		str 	r2,[r4]			; x=r2
				str 	r3,[r5]			; y=r3
				ldr 	r1,[fp,#8]		; r1=valor del ascii
				strb 	r1,[r8]			; inicio_pan+posicion='X'
				
				POP 	{r0-r8,r11,pc}	; desapilar registros, r11 y pc

buscar_libre	; mirar si algun hueco alrededor esta libre

				; mirar a la derecha
				ldr 	r2,[r4]			; r2=x
				ldr 	r3,[r5]			; r3=y
				add 	r2,r2,#1		; r2=r2+1
				and 	r2,#31			; empieza por el otro lado (x en modulo 31)
				LDR 	r6,=tam_fila	; r6=32
				mul 	r7,r3,r6		; r7=r3*r6=y*32
				add 	r8,r7,r2		; r8=r3*r6+r2=y*32+x	
				add 	r8,r0,r8		; r8=@inicio_pan+r8
				...
				mov 	r1,#0			; r8=0
				cmp 	r7,#' '			; if r7 != ' '
				beq		cambio_dir		; saltar a fin dibujar
				...
				ldr 	r3,[r5]			; r3=y
				add 	r3,r3,#1		; r3=r3+1
				and 	r3,#15			; empieza por el otro lado (y en modulo 15)
				LDR 	r6,=tam_fila	; r6=32
				mul 	r7,r3,r6		; r7=r3*r6=y*32
				add 	r8,r7,r2		; r8=r3*r6+r2=y*32+x	
				add 	r8,r0,r8		; r8=@inicio_pan+r8
				...
				mov 	r1,#1			; r8=1
				cmp 	r7,#' '			; if r7 != ' 
				beq 	cambio_dir		; saltar a fin_dibujar
...
cambio_dir		ldr 	r0,[fp,#28]		; r0=@dirx
				ldr 	r7,[fp,#32]		; r7=@diry
				strb	r6,[r0]			; dirx=r6
				strb	r1,[r7]			; diry=r8
				b 		fin_dibujar		; saltar a fin_dibujar
