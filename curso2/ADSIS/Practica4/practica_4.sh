...
remote_del() {
    local ip="$1" user="$2"
    ssh "${SSH_OPTS[@]}" as@"$ip" "sudo bash -s" << ENDSSH
# Crear siempre el directorio de backup
...
tar -cf /extra/backup/${user}.tar -C /home "$user" 2>/dev/null
if [ \$? -eq 0 ]; then
    userdel -r "$user" 2>&1 | grep -v "mail spool" >/dev/null
else
...
while read -r -u3 IP; do
    ...
    if ! ssh "${SSH_OPTS[@]}" as@"$IP" "exit" 2>/dev/null; then
        print_log "$IP no es accesible"
        continue
    fi

    while IFS="," read -r -u4 user pass name; do
        ...
        if [ "$ACCION" = "-a" ]; then
            ...
            # Imprimir solo el resultado (sin prefijo IP) para que los tests lo capturen
            echo "$RESULT"
            echo "$RESULT" >> "$LOG"

        elif [ "$ACCION" = "-s" ]; then
            remote_del "$IP" "$user" 2>/dev/null
        fi
