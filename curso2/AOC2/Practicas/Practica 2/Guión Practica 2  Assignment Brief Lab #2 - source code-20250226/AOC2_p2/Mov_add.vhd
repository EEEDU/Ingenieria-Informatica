...
-- Example: We can connect PC_out with the ADDR of the ROM memory writing: Mem_I: memoriaROM_I PORT MAP (ADDR => PC_out,  	
	...
	Mem_I: memoriaROM_I PORT MAP (ADDR => open, RE => '1', Dout => Instruction);
					
	UC: UC_Mov_Add PORT MAP (clk => clk, reset => reset, op_code => open, PC_ce => PC_ce, load_A => load_A, load_B => load_B, load_ALUout => load_ALUout, RegWr => RegWrite, MUX_ctrl => MUX_ctrl);
	
	Register_bank: BReg PORT MAP (clk => clk, reset => reset, RA => open, RB => open, RW => open, BusW => BusW, RegWrite => RegWrite, BusA => BusA, BusB => BusB);
	
	Reg_A: reg generic map (size => 32)
			port map (	Din => open, clk => clk, reset => reset, load => load_A, Dout => RA_out);
			
	Reg_B: reg generic map (size => 32)
			port map (	Din => open, clk => clk, reset => reset, load => load_B, Dout => RB_out);
	
	adder: adder32 port map (Din0 => open, Din1 => open, Dout => Adder_out);
	
	ALUout: reg generic map (size => 32)
			port map (	Din => open, clk => clk, reset => reset, load => load_ALUout, Dout => ALU_out);
	...
	mux: mux2_1 generic map (size => 32) port map (Din0 => open, Din1 => open, ctrl => MUX_ctrl, Dout => BusW);
