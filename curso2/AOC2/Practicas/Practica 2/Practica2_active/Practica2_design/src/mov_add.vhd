...
-- Example: We can connect PC_out with the ADDR of the ROM memory writing: Mem_I: memoriaROM_I PORT MAP (ADDR => PC_out,  	
	...
	Register_bank: BReg PORT MAP (clk => clk, reset => reset, RA => Instruction(20 downto 16), RB => Instruction(15 downto 11), RW => Instruction(25 downto 21), BusW => BusW, RegWrite => RegWrite, BusA => BusA, BusB => BusB);
	...
	mux: mux2_1 generic map (size => 32) port map (Din0 => K_ext, Din1 => ALU_out, ctrl => MUX_ctrl, Dout => BusW);
