...
begin
	...
	-- SOLUCION: 
		-- load_pc = 1 si no se ha parado el MIPS ni el ID
	...
	-- SOLUCION:
		-- Si hay excepcion Data abort, se salta a la direccion 0x00000008
		-- Si hay excepcion UNDEF, se salta a la direccion 0x0000000C
		-- Si hay excepcion es IRQ, se salta a la direccion 0x00000004
		-- Si es una operacion RTE, se salta a la direccion que se guardó en la cabeza de pila (Exception_LR_output)
		-- Si es una operacion RET, se salta a la direccion que se guardó en el Rs del banco de registros (busA)
		-- Si es una operacion JAL o se toma el salto de BEQ, se salta a la direccion que toque, depende de operacion (Dirsalto_ID)
		-- Sino se salta a la siguiente instruccion (PC+4)
	...
	---------------------------------------------------------------------------------
	-- NEW
	-- Incomplete integer forwarding unit (Unidad de anticipación). You must design it taking into account which registers each instruction reads and writes.
	...
	Banco_EX_MEM: Banco_MEM PORT MAP ( 	ALU_out_EX => ALU_out_EX, ALU_out_MEM => ALU_out_MEM, clk => clk, reset => reset_MEM, load => load_MEM, MemWrite_EX => MemWrite_EX,
										...
										-- SOLUCION: Usamos la salida del mux de anticipacion
										BusB_EX => Mux_B_out,  
										--FIN COMPLETAR
...
-------------------------------------------------------------------------------------------------- 
	...
	-- Complete:
	-- SOLUCION: 
		-- Contador de instrucciones ejecutadas aumenta si la instrucción en WB es válida.
		-- Contador de excepciones aumenta si se ha aceptado la excepcion
		-- Contador de paradas por datos aumenta si se ha parado en ID pero no en el MIPS
		-- Contador de paradas por control aumenta si se ha matado un instruccion en IF, cuando no ha para do ID ni MIPS
		-- Contador de paradas por memoria aumenta si se ha parado el MIPS
	inc_cycles <= '1';--Done
	inc_I <= valid_I_WB AND not(stall_MIPS); --Done
	inc_Exceptions <= Exception_accepted;
	inc_data_stalls <= stall_ID and not(inc_MEM_stalls);
	inc_control_stalls <= kill_IF and not(inc_data_stalls) and not(inc_Mem_stalls);
	inc_Mem_stalls <= not(IO_MEM_ready);
