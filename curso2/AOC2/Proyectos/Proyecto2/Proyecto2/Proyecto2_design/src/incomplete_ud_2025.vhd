...
-- SOLUCION:
	-- Se mata a la instruccion en IF, cuando es un salto incondicional o cuando el salto condicional sea tomado, siempre y cuando la instruccion sea valida en ID y no tenga riesgos de datos 
	Kill_IF <= 	'1' when (valid_I_ID = '1' AND riesgo_datos_ID = '0' AND ((IR_op_code = JAL_opcode) or (IR_op_code = RET_opcode) or (IR_op_code = RTE_opcode) or ((IR_op_code = BEQ_opcode) AND (salto_tomado = '1')))) 
					else '0';
-------------------------------------------------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------------------------------------------------
-- Data dependencies:
...
-- SOLUCION:
	-- Rs lee en instrucciones: ARIT, LW, SW, BEQ, RET y FI
	-- Rt lee en instrucciones: ARIT, SW y BEQ
	...
	rt_read <= '1' when ((IR_op_code = ARIT_opcode) or (IR_op_code = SW_opcode) or (IR_op_code = BEQ_opcode) or (IR_op_code = FI_opcode)) else '0'; --complete
	-- Conditions for each dependency:
	-- Notation: dep_rs_EX: data dependecy in Rs, with the instruction in EX stage.
	dep_rs_EX 	<= 	'1' when ((valid_I_EX = '1') AND (valid_I_ID = '1') AND (Reg_Rs_ID = RW_EX) and (RegWrite_EX = '1') and (rs_read = '1'))	
						else '0';
	--Complete:
	dep_rs_Mem	<=  '1' when ((valid_I_MEM = '1') AND (valid_I_ID = '1') AND (Reg_Rs_ID = RW_MEM) and (RegWrite_MEM = '1') and (rs_read = '1'))	
						else '0';
							
	dep_rt_EX	<= 	'1' when ((valid_I_EX = '1') AND (valid_I_ID = '1') AND (Reg_Rt_ID = RW_EX) and (RegWrite_EX = '1') and (rt_read = '1'))
						else '0';
								
	dep_rt_Mem	<= 	'1' when ((valid_I_MEM = '1') AND (valid_I_ID = '1') AND (Reg_Rt_ID = RW_MEM) and (RegWrite_MEM = '1') and (rt_read = '1'))
						else '0';
...
-- Data hazards:
	-- 1) lw_uso: 
	-- SOLUCION:
		-- Load uso ocurre cuando la instruccion en EX es una carga y el registro es dependiente de la con la instruccion en el estado EX
		-- Ocurre lo mismo con lso dos registros 
	ld_uso_rs <= 	'1' when ((MemRead_EX = '1') AND (dep_rs_EX = '1'))
						else '0';
	ld_uso_rt <= 	'1' when ((MemRead_EX = '1') AND (dep_rt_EX = '1'))
						else '0';	
									
	-- 2) BEQ: BEQ reads the registers in ID, and we do not have a forwarding network in that stage
	-- SOLUCION:
		-- Esto ocurre cuando la instraccion es un BEQ y alguno de los registros es dependiendiente e alguna de las etapas. Por lo que no podemos compararlos
	BEQ_rs	<= 		'1' when ((IR_op_code = BEQ_opcode) AND ((dep_rs_EX = '1') or (dep_rs_Mem = '1')))
						else '0';
	BEQ_rt	<= 		'1' when ((IR_op_code = BEQ_opcode) AND ((dep_rt_EX = '1') or (dep_rt_Mem = '1')))
						else '0';
		
	-- 3) RET: Similar to beq hazard, but taking into account that RET only uses Rs
	-- SOLUCION:
		-- Ocurre algo paracido al BEQ, si es una instruccion RET y el resgistro rs es dependiente. Por lo que aun no se puede usar
	RET_rs	<= 		'1' when ((IR_op_code = RET_opcode) AND ((dep_rs_EX = '1') or (dep_rs_Mem = '1')))
						else '0';
	...
	-- SOLUCION:
		-- Ocurre cuando la instruccion quiere acceder a un registro que ha utilizado una instruccion hace 1 o 2 ciclos. Deber ser valida la instruccion y querer leer el registro
	JAL_uso_rs <= '1' 	when (((JAL_EX = '1') AND (Reg_Rs_ID = RW_EX) AND (valid_I_EX = '1')) OR ((JAL_MEM = '1') AND (Reg_Rs_ID = RW_MEM) AND (valid_I_MEM = '1'))) AND (rs_read = '1') AND (valid_I_ID = '1')
						else '0';

	JAL_uso_rt	<= 	'1' when (((JAL_EX = '1') AND (Reg_Rt_ID = RW_EX) AND (valid_I_EX = '1')) OR ((JAL_MEM = '1') AND (Reg_Rt_ID = RW_MEM) AND (valid_I_MEM = '1'))) AND (rt_read = '1') AND (valid_I_ID = '1')
						else '0';
