...
{
    // sync.sPasarTestigo.wait();
    int estudiante = 0;
    bool pasadoTestigo = false;

    // cout << "La silla tiene  " << datos.iSilla << " huecos y se esta eperando por " << sync.dSillaVacia << " procesos " << endl;
    ...
    {
        // cout << "----------- entra uno a la silla porque hay " << datos.iSilla << endl;
        sync.dSillaVacia--;
    ...
    {   
        if (datos.tienePareja[estudiante] == true && sync.dTienePareja[estudiante] > 0)
        {
    ...
    if ((datos.estudianteTerminado >= N_EST) && sync.dFinEstudiantes > 0 && !pasadoTestigo)
    {
    ...
    else if (pasadoTestigo == false)
    {
        sync.testigo.signal();
    }
    // sync.sPasarTestigo.signal();
...
void Estudiante(int nip, DatosCompartidos& datos, Sincronizacion& sync)
{
    ...
    // iSilla++ >
    sync.testigo.wait();
    // cout << nip << " quiere entrar en la silla y hay " << datos.iSilla << endl;
    if (datos.iSilla >= 2)
    {
        sync.dSillaVacia++;
        sync.testigo.signal();
    ...
    // cout << datos.nipSilla[datos.iSilla] << " ha entrado en silla " << datos.iSilla << endl;
    datos.iSilla++;
    ...
    // cout << "Estudiante " << nip << " tiene pareja " << datos.pareja[nip] << " con la fila" << filaEstudiante << endl;
    ...
    else
    {
        ...
        // < estudianteTerminado ++ >
        sync.testigo.wait();
        datos.estudianteTerminado ++;
        // cout << "++++++++++++++++ " << nip << " ha terminado " << endl;
        pasarTestigo(sync, datos);
}
// ----------------------------------------------------
void Profesor(DatosCompartidos& datos, Sincronizacion& sync)
{
    for (int i = 0; i < N_FIL; i++)
    {
        ...
        // cout << "Ya hay una pareja y es: " << datos.nipSilla[0] << " y " << datos.nipSilla[1] << endl;
        ...
        // cout << "******* libero la silla" << datos.iSilla << endl;
        datos.iSilla = 0;
        pasarTestigo(sync, datos);
    }
...
int main()
{
    ...
    Semaphore testigo(1), sSillaVacia(0), sSillaOcupada(0), sFinEstudiantes(0), sPasarTestigo(1);
    ...
    Sincronizacion sync = { testigo,                // Semaphore& testigo      
                            ...
                            sPasarTestigo};         // semaphore& sPasarTestigo
