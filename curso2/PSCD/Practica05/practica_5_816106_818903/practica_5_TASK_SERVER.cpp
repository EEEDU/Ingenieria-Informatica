...
//-------------------------------------------------------------
void servCliente(Socket& chan, int client_fd, ControlBuffer &controlBuffer) {
    ...
    while(!out) {
        ...
        if (buffer == MENS_FIN || buffer == "END") {
            out = true; // Salir del bucle
        } 
        else if (buffer == "GET_TASK") {
            Tarea tareaController = controlBuffer.getTask();

            // Enviamos la respuesta
            string s;
            if (tareaController.tipoTarea == TAREA_FINAL) s = TAREA_FINAL;
            else s = tareaController.tipoTarea + "," + to_string(tareaController.cargaDeTrabajo);

            int send_bytes = chan.Send(client_fd, s);
            cout << "Tarea enviada: " + s + "\n";
...
//-------------------------------------------------------------
void master(string archivoTareas, ControlBuffer &controlBuffer, bool &final)
{
    Tarea tarea;
...
int main(int argc,char* argv[]) {
    ...
    thread master_thread(&master, fichero, ref(controlBuffer), ref(final_fichero));
    ...
    while (!fin && i < 10) {
        ...
        if(new_client_fds == -1) {
        	...
        	if (!fin) {
        	    ...
        	    cliente.push_back(thread(&servCliente, ref(chan), new_client_fds, ref(controlBuffer)));
